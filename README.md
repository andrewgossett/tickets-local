...
1. [Download the Windows x64 application](https://github.com/andrewgossett/tickets-local/releases/download/v0.5.18/Tickets-Local-0.5.18-Windows-x64.exe), or download and unzip the [portable Windows ZIP](https://github.com/andrewgossett/tickets-local/releases/download/v0.5.18/Tickets-Local-0.5.18-Windows-x64-unsigned.zip).
...
An enrolled responder can explicitly update their position from the companion
using the phone's browser location permission, or search for an address or two
cross streets and select the matching result. Location is never collected in
the background. Browser GPS access may require a secure browser context; the
cross-street option remains available when the phone browser blocks GPS on a
plain trusted-LAN HTTP address.
